...
#added_x1= np.full((N,1),5090)
#modified_x=np.append(xdata,added_x1,axis=1)
...
#added_x2= np.full((v_shape_x,1),5090)
#modified_test=np.append(vxdata,added_x2,axis=1)
...
if len(sys.argv) == 1:
    batch_size = modified_x.shape[0] // 10 # 10 batches
else:
...
with tf.Session() as session:
    ...
    for epoch in range(n_epochs):
        mini_batches = create_mini_batches(modified_x,
     ...
     # check accuracies training and test
            ...
            acc_val = accuracy.eval(feed_dict={X:modified_test, t:vtdata})
            if(maxx_acc<acc_val):
                maxx_acc=acc_val
...
print("Neurons in Hidden layer2: ",n_hidden3 )
print("Neurons in Hidden layer2: ",n_hidden4 )

print("No. of input samples: ", N)
print("Accuracy",acc_val)
print('DNN Model default+const 5090, Max accuracy  at epoch no. : ',
