...
#    print(epoch_for_maxx, max_acc_train, maxx_acc)
...
print('DNN Model With default settings, Converged at epoch no. : ',
...
with tf.Session() as session:
    ...
    for epoch in range(n_epochs):
     ...
     # check accuracies training and test
            acc_train = accuracy.eval(feed_dict={X:X_batch, t:t_batch})
            acc_val = accuracy.eval(feed_dict={X:vxdata, t:vtdata})
            if(maxx_acc2<acc_val):
                maxx_acc2=acc_val
                epoch_for_maxx2=epoch
                max_acc_train2=acc_train
#            save_path = saver.save(session,"./model_final.ckpt")    
...
print('DNN Model With HE, Max accuracy  at epoch no. : ',
              epoch_for_maxx2, '   Maximum Accuracy of the model : ', maxx_acc2)

if (maxx_acc2>maxx_acc):
    print("Number 4 performed better than number 1")
elif (maxx_acc2<maxx_acc):
    print("Number 1 performed better than number 4")
else:
    print("Both provide same accuracy")
